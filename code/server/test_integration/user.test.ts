...
describe("User routes integration tests", () => {
    describe("POST /users", () => {
        ...
        test("It should return a 409 error code if at least one request body parameter is empty/missing", async () => {
            await request(app)
                .post(`${routePath}/users`)
                .send(customer)
                .expect(409) 
    ...
    describe("GET /users/:username", () => {
        test("It should return an array of users with a specific username", async () => {
            const admins = await request(app).get(`${routePath}/users/admin`).set("Cookie", adminCookie).expect(200)
            expect(admins.body.username).toBe(admin.username)
            expect(admins.body.name).toBe(admin.name)
            expect(admins.body.surname).toBe(admin.surname)
        })
    ...
    describe("PATCH /users/:username", () => {
        test("It should return a 200 success code and create a new user", async () => {
            let newCustomer = {...customer, name: 'customer1', address: 'test', birthdate: '1990-01-01'}
            await request(app)
        ...
        test("It should fail if the birthdate is not valid", async () => {
            let newCustomer = {...customer, birthdate: '2990-01-01', address: 'test'}
            await request(app)
                .patch(`${routePath}/users/customer`)
                .send(newCustomer)
                .set("Cookie", adminCookie)
