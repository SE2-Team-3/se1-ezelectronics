...
import {InvalidBirthDateError, UserAlreadyExistsError, UserNotFoundError, UserNotManagerError, UserNotAdminError, UserNotCustomerError, UnauthorizedUserError, UserIsAdminError } from "../errors/userError";
...
class UserDAO {
        ...
        return new Promise<boolean>((resolve, reject) => {
            try {
                ...
                db.get(sql, [username], (err: Error | null, row: any) => {
                    ...
                    if (!row) {
                        reject(new UserNotFoundError()); // 404 error when username does not exist in the database
                        return;
                    }
                    ...
                    // 401 error when the calling user is an Admin and username represents a different Admin user
                    if (user.role === "Admin" && user.username !== username && row.role === "Admin") {
                        reject(new UnauthorizedUserError());
                        return;
                    }
     ...
     */
    updateUserInfo(user: User, username: string, name: string, surname: string, address: string, birthdate: string): Promise<User> {
        return new Promise<User>((resolve, reject) => {
            try {
                // We also check if birthdate is after current date
                if (new Date(birthdate) > new Date()) {
                    reject(new InvalidBirthDateError());
                    return;
                }
                
                
                // First, check if the user performing the update is allowed to do so
                if (user.username !== username && user.role !== "Admin") {
                ...
                // Retrieve the current role of the user being updated to check for admin-specific constraints
                db.get("SELECT role FROM users WHERE username = ?", [username], (err: Error | null, row: any) => {
                    if (err) {
                        reject(err);
                        return;
                    }
