...
class UserController {
     ...
     * @param user - The logged-in user calling the route.
    ...
    async getUserByUsername(user: User, username: string): Promise<User> {
        ...
        // Check if the requesting user is allowed to access the requested user
        if (user.username !== username && !Utility.isAdmin(user)) {
    ...
    async updateUserInfo(user: User, name: string, surname: string, address: string, birthdate: string, username: string) /**:Promise<User> */ { 
        return this.dao.updateUserInfo(user, username, name, surname, address, birthdate);
    }
}
