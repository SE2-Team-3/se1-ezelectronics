...
class ProductController {
    ...
    async registerProducts(model: string, category: string, quantity: number, details: string | null, sellingPrice: number, arrivalDate: string | null): Promise<void> {
        try {
            await this.dao.registerProducts(model, category, quantity, details, sellingPrice, arrivalDate);
        } catch (error) {
            if (error instanceof InvalidParameterCombinationError) {
                throw new InvalidParameterCombinationError();
            }
            throw error;
        }
    }
    ...
    async changeProductQuantity(model: string, newQuantity: number, changeDate: string | null): Promise<number> { 
        try {
            const updatedQuantity = await this.dao.changeProductQuantity(model, newQuantity, changeDate);
            ...
            } else if (error instanceof ChangeDateInFutureError) {
                throw new ChangeDateInFutureError();
            } else if (error instanceof ChangeDateBeforeArrivalError) {
                throw new ChangeDateBeforeArrivalError();
            }
            throw error;
        }
    }
